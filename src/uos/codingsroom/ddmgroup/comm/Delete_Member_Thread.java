...
public class Delete_Member_Thread extends Communication_Thread {

	// 생성자 (글 등록하기)
	public Delete_Member_Thread(Context context, int menu, int mem_num) {
		super(context,menu);
		url += "&mem_num=" + mem_num;
	}
		...
		try {
			...
			while (eventType != XmlPullParser.END_DOCUMENT) { // xml이 끝날때까지 계속 돌린다.
				if (eventType == XmlPullParser.START_TAG) {
					...
					if (tagname.equals("MODE") || tagname.equals("MEMBER")) {
