...
public class Delete_Notice_Thread extends Manage_Communication_Thread {

	// 생성자 (글 등록하기)
	public Delete_Notice_Thread(Context context, int menu, int notice_num) {
		super(context, menu);
		...
		try {
			...
			while (eventType != XmlPullParser.END_DOCUMENT) { // xml이 끝날때까지 계속 돌린다.
				if (eventType == XmlPullParser.START_TAG) {
					...
					if (tagname.equals("MODE") || tagname.equals("NOTICE")) {
